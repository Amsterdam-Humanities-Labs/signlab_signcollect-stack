...
|---|---|---|---|---|
| hh pipeline output: `data/` (crawled health pages, PDFs, saved HTML, subtitles, `wordlist.txt`) | removed from `master` in `c84c448`; still on hh branch `install-root`; bundle (ref `c84c448^`) | 4,469 files, 297 MB | No. Public health information, third-party copyright | TODO: confirm |
| zin database dump and work sheets: `sentences_backup_20260225.sql`, `zinnen_feed.json`, `cache/zinnen_feed.json`, `znn_videos/all_videos.json`, 3 `.xlsx` | bundle only | 8 files, 16.8 MB | Unknown. Check the dump for user names before upload | TODO: confirm |
| zin one-off reports: `deleted_klaar_gloss.json`, `unmatched_sentences.json`, `annotatie_check_userlist.csv` and 7 more | zin branch `install-root` (removed from `main`) | 10 files, 0.9 MB | Pseudonymous (video IDs) | TODO: confirm |
| zin source sentence lists: `2k.csv`, `2k_enhanced.csv`, `zin.csv`, `Extra_2000zinnen.csv`, `extrazinnen_v1.csv`, `lorraine_sentences.csv` | zin `main` (not archived yet; no `archive-19` branch exists) | 6 files, 0.54 MB | No (sentences) | TODO: confirm |
...
| s3b_server hand-cluster plots: `centroids/`, `visualizations/`, `api/cache.json` | bundle only | 29 files, 16.7 MB | No (plots) | TODO: confirm |
| drs DaVinci project: `old/lala6_project.zip` | drs `main` | 36.4 MB | Low: 174 recording file paths, no media | TODO: confirm |
...
| videoBackgroundFix previews `tmp/*.mp4`, `tmp/*.png` | bundle only | 20 files, 23.6 MB | Previews of studio recordings, so likely identifiable participants. Derived; the originals are on the research drive |
| zin `znn_videos/test_output/R20251209_1983.mp4` | zin branch `install-root` | 1.5 MB | Same |
...
|---|---|---|
| hh `odwn_orbn_gwg-LMF_1.3.xml.gz` | 15.3 MB | Open Dutch WordNet download |
| annotation-tool `clusters/tool/glosses_transformed.json` | 11.0 MB | Signbank export, regenerated |
| zin `MANO_LEFT.pkl`, `MANO_RIGHT.pkl` | 7.6 MB | MANO hand model, from its publisher |
| drs `yolov8n.pt` | 6.5 MB | Ultralytics release |
| Sony-SDK `external/**/*.dylib`, sC-Animation-PP `vendor/`, `composer.phar` | 31 MB | Vendor SDK, Composer |
...
Result for hh `data/` (tested 2026-09-23): 4,469 files, 297 MB, 111 MB as
