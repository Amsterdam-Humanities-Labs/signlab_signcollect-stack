...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $sqla = "SELECT camera1, camera2, camera3, camera4, camera5, videoTop, datetime_ms, user FROM CameraRecords  WHERE zOg='nmm' AND glosId = ? AND (user LIKE ? OR ? = '%') AND stateVideo = 'stopped'";
        ...
        $response = [
            ...
            'glos' => $row['glos'],
            'zelfopname' => $zelfopname, // URL to the video
            'type' => $row['type'],
            'videos' => $videos
        ];
